...
# 노드 추가 및 레이블 설정
for i, (point, label) in enumerate(zip(points, semantic)):
    G.add_node(i, pos=(point[0], point[1]), label=label)

# 엣지 추가 (마지막 노드를 첫 번째 노드와 연결)
for i in range(len(points)):
    G.add_edge(i, (i+1) % len(points))

# 그래프 저장
nx.write_gpickle(G, 'your_graph.gpickle')

# 그래프 로드
G = nx.read_gpickle('your_graph.gpickle')
...
# 색상 매핑에 따른 노드 색상 설정, 레이블이 없는 노드에 대한 예외 처리 추가
node_colors = [color_mapping.get(str(labels.get(node, default_label)), '#000000') for node in G.nodes()]

# 그래프 그리기
plt.figure(figsize=(8, 6))
nx.draw_networkx(G, pos, with_labels=True, node_color=node_colors, edge_color='black', node_size=50)
plt.show()