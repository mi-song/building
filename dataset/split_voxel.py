...
# List all files in the directory and extract the desired part of the filenames
try:
    # Listing files in the directory
    file_names = os.listdir(folder_path)

    # Extracting the desired part from each filename
    extracted_names = [filename.split('_label.json')[0] for filename in file_names if filename.endswith('_label.json')]
except FileNotFoundError:
    extracted_names = "The specified folder does not exist or the path is incorrect."

# file_names_without_extension = [os.path.splitext(os.path.basename(file))[0] for file in ply_files]
file_names_without_extension = [os.path.splitext(os.path.basename(file))[0] for file in extracted_names]
...
for file_name in tqdm(file_names):
    ...
    pickle_folder_path = os.path.join(root_path, 'voxel_data_v2', str(voxel_size))
